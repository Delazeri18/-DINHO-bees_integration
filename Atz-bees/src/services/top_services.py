...
def consultar_status_pedidos(token, status_igual=None, status_diferente=None, pedido_id=None,
                              data_inicio=None, data_fim=None, sistema_integrado=None):
    url = ""
    ...
    if status_igual:
        params["StatusIgual"] = status_igual
    if status_diferente:
        params["StatusDiferente"] = status_diferente
    if pedido_id:
        params["Pedido"] = pedido_id
    if data_inicio:
        params["DataInicioPedidoStatus"] = data_inicio
    if data_fim:
        params["DataFimPedidoStatus"] = data_fim
    if sistema_integrado:
        params["SistemaIntegrado"] = sistema_integrado
    ...
    try:
        ...
        if response.status_code == 200:
            dados = response.json()
            if not dados:
                print("Nenhum pedido retornado.")
                return False, None
            
            # Seleciona o último item da lista
            ultima_atz = dados[-1]
            data_hora = ultima_atz.get("dataHora")
            status = ultima_atz.get("status")
            
            print(f"Última inserção - Data/Hora: {data_hora}, Status: {status}")
            return True, {"dataHora": data_hora, "status": status}
        
        elif response.status_code == 401:
            print({"error": "Unauthorized - Verifique seu token ou permissões."})
            return False, response.json()
        else:
            print({"error": f"Erro inesperado: {response.status_code}", "detalhes": response.text})
            return False, response.json()
