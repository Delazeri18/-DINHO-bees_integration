...
class BigQuery:
    ...
    def ler_dados(self):
        ...
        WHERE 
            max_days IS NOT NULL     AND
            id_topsytem IS NOT NULL
        """ 
    ...
    def update_order(self, dados):
        ...
        try:
            ...
            query_job = self.client.query(query)
            query_job.result()  # Aguarda a conclusão da consulta
            logger.info(f"Pedido com id_bees='{dados['id_bees']}' atualizado com sucesso.")
        except Exception as e:
            logger.info(f"Erro ao atualizar pedido no BigQuery: {e}")
